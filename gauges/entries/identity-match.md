...
ratchet: "n/a (already C); if no GT render exists, drop to Predicted (a preference model) and log human 'same/different' calls to build the GT set"
...
source: "a character-reconstruction project - replaced 'does it look like them?' (Reviewed tier on a video) with a per-pose number"
---
