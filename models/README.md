...
`index.json` catalogs models that **encode human responses** — the tier-B proxies
(and the backbones for tier-C feature distances). Use it to pick a proxy by
...
3. Decide `use_as`:
   - **C-metric** — distance to a *target* (e.g. LPIPS/ArcFace cosine to a reference). Preferred.
   - **B-proxy** — preference/quality score when there's no target (e.g. PickScore, reward models).
4. Always add a cross-check (a second model or an orthogonal metric).

## Calibration is part of the score

A B-proxy is only as good as its agreement with real human labels. When you
